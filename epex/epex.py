...
class Marketprice:
    ...
    def price_eur_per_kwh(self):
        return self._price_eur_per_mwh / 1000
...
class EPEXSpotWeb:
    ...
    def fetch_json(self):
        ...
        data = []
        for price in self.marketdata:
            data.append({
                "timestamp": price.start_time.strftime('%Y-%m-%dT%H:%M:%S'),
                "price_eur_per_kwh": price.price_eur_per_kwh
            })
